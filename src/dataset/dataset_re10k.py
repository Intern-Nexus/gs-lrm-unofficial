...
from ..geometry.pose_utils import centerize_scale_poses
...
class DatasetRE10k(IterableDataset):
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for example in chunk:
                extrinsics, intrinsics = self.convert_poses(example["cameras"]) #c2w
                scene = example["key"]
                ...
                using_extrinsics = extrinsics[torch.cat([context_indices, target_indices])]
                ...
                #     scale = 1
                # breakpoint()
                # avg_c2w = poses_avg(using_extrinsics)
                using_extrinsics, scale = centerize_scale_poses(using_extrinsics)
                # breakpoint()
                

                example = {
                    "context": {
                        "extrinsics": using_extrinsics[:2],
    ...
    def convert_poses(
        ...
        return w2c.inverse(), intrinsics.clone()
